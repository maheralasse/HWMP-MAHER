...
HwmpProtocol::DoDispose ()
{
  ...
  m_hwmpSeqnoMetricDatabaseMP.clear ();         ///////////////
  m_interfaces.clear ();
  m_rqueue.clear ();
  m_rtable = 0;
  m_mp = 0;
...
HwmpRtable::LookupResult result = m_rtable->LookupMinimumTheProcessingDelay (destination,packet);           ///////
...
HwmpProtocol::ReceivePreq (IePreq preq, Mac48Address from, uint32_t interface, Mac48Address fromMp, uint32_t metric)
{
  ...
  //acceptance cretirea:
	...
	std::map<std::pair<Mac48Address, Mac48Address>, std::pair<uint32_t, uint32_t> >::const_iterator j =
			m_hwmpSeqnoMetricDatabaseMP.find (std::make_pair(preq.GetOriginatorAddress (), from));   
			
   bool freshInfoMP (true);
	
		if (j != m_hwmpSeqnoMetricDatabaseMP.end ())  
		{     
               if ((int32_t)(j->second.first - preq.GetOriginatorSeqNumber ())  > 0)
			{
				return;
			}		
	    		if ((j->second.first == preq.GetOriginatorSeqNumber ())
			
			{
				freshInfoMP = false;
				
				if (j->second.second <= preq.GetMetric ())
				{
					return;
				}
  ...
  m_hwmpSeqnoMetricDatabaseMP[std::make_pair(preq.GetOriginatorAddress (), from)]
  ...
  if (
			(freshInfoMP) || 
			(
			(m_rtable->LookupReactive (preq.GetOriginatorAddress (),from).retransmitter == Mac48Address::GetBroadcast ()) && 
		    (m_rtable->LookupReactive (preq.GetOriginatorAddress (),from).bandwidth_required >= preq.GetBandwidth_required ())  &&              ////////////
            (m_rtable->LookupReactive (preq.GetOriginatorAddress (),from).processing_delay_required <= preq.GetProcessing_delay_required ())  //////////
			)
	 )           
  ...
  if (
    (m_rtable->LookupReactive (fromMp,from).retransmitter == Mac48Address::GetBroadcast ()) ||
    (m_rtable->LookupReactive (fromMp,from).metric > metric)
    )
    {
      m_rtable->AddReactivePath (
        fromMp,
	...
	NS_LOG_UNCOND ("Routing Table For Address = " << GetAddress () << ", Accepted preq from address" << from << ", preq:" << preq);
      ...
      if ((*i)->GetDestinationAddress () == Mac48Address::GetBroadcast ())
        {
          ...
          ((m_rtable->LookupProactive ()).processing_delay_required < preq.GetProcessing_delay_required ())&&                          
          ((m_rtable->LookupProactive ()).bandwidth_required >= preq.GetBandwidth_required ())          ////////
            )
            {
      ...
      if ((*i)->GetDestinationAddress () == GetAddress ())
        {
          ...
          NS_ASSERT (m_rtable->LookupReactive (preq.GetOriginatorAddress (),from).retransmitter != Mac48Address::GetBroadcast ());
          ...
          if ((lifetime > 0) && ((int32_t)(result.seqnum - (*i)->GetDestSeqNumber ()) >= 0))
            {
              ...
              m_rtable->AddPrecursor ((*i)->GetDestinationAddress (), interface, from,
                                      MicroSeconds (preq.GetLifetime () * 1024),preq.GetPathId ());
              if ((*i)->IsRf ())
                {
...
HwmpProtocol::ReceivePrep (IePrep prep, Mac48Address from, uint32_t interface, Mac48Address fromMp, uint32_t metric)
{
  ...
  prep.IncrementMetric (metric);
		...
		std::map<std::pair<Mac48Address, Mac48Address>, std::pair<uint32_t, uint32_t> >::const_iterator j =
				m_hwmpSeqnoMetricDatabaseMP.find (std::make_pair(prep.GetOriginatorAddress (), from);
		bool freshInfoMP (true);
		uint32_t sequence = prep.GetDestinationSeqNumber ();
		if ((j != m_hwmpSeqnoMetricDatabaseMP.end ())&&(m_rtable->checkPathId(destination,pathid)==true))        ///////////
		{
		  if (((int32_t)(j->second.first - sequence) > 0)
			{
			  return;
			}
		  if (j->second.first == sequence)
			{
			  freshInfoMP = false;
			}
		}
	m_hwmpSeqnoMetricDatabaseMP[std::make_pair(prep.GetOriginatorAddress (), from)]
  ...
  //metric
    ...
    if (
			(freshInfoMP) ||
			(
			((m_rtable->LookupReactive (prep.GetOriginatorAddress (),from)).pathid == prep.GetPathId ())&&            ///////////
		    ((m_rtable->LookupReactive (prep.GetOriginatorAddress (),from)).retransmitter == Mac48Address::GetBroadcast ()) ||
			((m_rtable->LookupReactive (prep.GetOriginatorAddress (),from)).metric > prep.GetMetric ())
			)
			)
		   ...
		   m_rtable->AddReactivePath (
			 ...
			 m_rtable->AddPrecursor (prep.GetDestinationAddress (), interface, from,
                              MicroSeconds (prep.GetLifetime () * 1024), prep.GetPathId ());
      if (result.retransmitter != Mac48Address::GetBroadcast ())
        {
          m_rtable->AddPrecursor (prep.GetOriginatorAddress (), interface, result.retransmitter,
                                  result.lifetime,prep.GetPathId ());
        }
      ReactivePathResolved (prep.GetOriginatorAddress ());
	  
  if (
    ((m_rtable->LookupReactive (fromMp,from)).pathid == prep.GetPathId ())&&          ////////////////
    ((m_rtable->LookupReactive (fromMp,from)).retransmitter == Mac48Address::GetBroadcast ()) ||
    ((m_rtable->LookupReactive (fromMp,from)).metric > metric)                               
    )
    {
...
HwmpProtocol::ReactivePathResolved (Mac48Address dst)
{
  ...
  if (i != m_preqTimeouts.end ())
   ...
   HwmpRtable::LookupResult result= m_rtable->LookupReactive (dst,packet.pkt);  //////
...
HwmpProtocol::SendProactivePreq ()
{
  ...
  preq.SetBandwidth_required (bandwidth_required);                            /////////////////////
  preq.SetProcessing_delay_required (processing_delay_required);            /////////////////////
  preq.SetPathId  (pathid);                    /////////////////////
  for (HwmpProtocolMacMap::const_iterator i = m_interfaces.begin (); i != m_interfaces.end (); i++)
    {
      i->second->SendPreq (preq);
    }
